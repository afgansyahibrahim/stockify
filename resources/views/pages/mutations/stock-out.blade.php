...
<div class="min-h-screen w-full bg-slate-50 p-4 sm:p-6 dark:bg-gray-900">
    ...
    <form id="stock-out-form" action="{{ route('stock.out.store') }}" method="POST">
        @csrf
                    ...
                    <div class="mt-5 space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="destination" value="{{ old('destination') }}" required
                                class="block w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm text-slate-900 focus:border-blue-500 focus:ring-4 focus:ring-blue-100"
                                placeholder="Contoh: Toko Gadget Jaya">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="reference_number" value="{{ old('reference_number') }}"
                                class="block w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm text-slate-900 focus:border-blue-500 focus:ring-4 focus:ring-blue-100"
                                placeholder="Nomor pesanan atau surat jalan">
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="notes" rows="4"
                                ...
                                placeholder="Alasan atau catatan pengeluaran.">{{ old('notes') }}</textarea>
                        </div>
                    </div>
                </div>

                <div class="rounded-xl border border-rose-200 bg-rose-50 p-4">
                    <p class="text-sm font-bold text-rose-800">Periksa stok tersedia</p>
                    <p class="mt-1 text-xs leading-relaxed text-rose-700">
                        Sistem menolak jumlah pengeluaran yang melebihi stok produk saat ini.
                    </p>
                </div>
            </div>
            ...
            <div class="xl:col-span-2">
                <div class="overflow-visible rounded-xl border border-slate-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    <div class="border-b border-slate-100 px-5 py-4 dark:border-gray-700">
                        <h2 class="text-base font-bold text-slate-900 dark:text-white">Produk yang Akan Dikeluarkan</h2>
                        <p class="mt-1 text-sm text-slate-500 dark:text-gray-400">
                            Cari produk dan isi jumlah yang akan dikeluarkan.
                        </p>
                    </div>
                            ...
                            <div id="product-menu"
                                ...
                                <div class="max-h-64 overflow-y-auto p-2">
                                    @foreach($products as $product)
                                        <button type="button"
                                            class="product-option flex w-full items-center justify-between gap-3 rounded-lg px-3 py-3 text-left hover:bg-blue-50 {{ $product->stock <= 0 ? 'cursor-not-allowed opacity-50' : '' }}"
                                            data-id="{{ $product->id }}"
                    ...
                    <div class="border-t border-slate-100 bg-slate-50 px-5 py-4">
                        <div class="ml-auto max-w-sm space-y-2">
                            ...
                            <div class="flex justify-between border-t border-slate-200 pt-3">
                                <span class="font-bold text-slate-900">Total Unit Keluar</span>
                                <span id="total-quantity" class="text-xl font-bold text-rose-600">0 Pcs</span>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function updateSummary() {
            ...
            rows.forEach(function (row) {
                quantityTotal += parseInt(row.querySelector('.item-quantity').value) || 0;
            });

            itemCount.textContent = rows.length + ' Produk';
            totalQuantity.textContent = quantityTotal + ' Pcs';
        ...
        function addProduct(id, name, sku, stock) {
            ...
            row.innerHTML = `
                ...
                <div class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
                    ...
                    <div>
                        <label class="mb-1.5 block text-xs font-semibold uppercase text-slate-500">Status Jumlah</label>
                        <div class="stock-status flex h-[42px] items-center rounded-lg bg-emerald-50 px-3 text-sm font-bold text-emerald-600">
                            Stok mencukupi
                        </div>
                    </div>
                </div>
            ...
            quantityInput.addEventListener('input', function () {
                ...
                if (quantity > parseInt(stock)) {
                    status.textContent = 'Melebihi stok tersedia';
                    status.className = 'stock-status flex h-[42px] items-center rounded-lg bg-rose-50 px-3 text-sm font-bold text-rose-600';
                } else {
                    status.textContent = 'Stok mencukupi';
                    status.className = 'stock-status flex h-[42px] items-center rounded-lg bg-emerald-50 px-3 text-sm font-bold text-emerald-600';
                }
        ...
        form.addEventListener('submit', function (event) {
            ...
            if (products.length === 0) {
                ...
                alert('Tambahkan minimal satu produk terlebih dahulu.');
                searchInput.focus();
                return;
            }

            const invalidQuantity = Array.from(document.querySelectorAll('.item-quantity'))
                .some(input => parseInt(input.value) > parseInt(input.max));
